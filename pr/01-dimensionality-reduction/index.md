...
Алгоритм метода LDA выглядит следующим образом:

**1. Посчёт суммы $$S_W$$ матриц рассеивания внутри классов.** Пусть у нас есть классы $$D_i = (\boldsymbol{x}_1^i, \boldsymbol{x}_2^i, \ldots, \boldsymbol{x}_{k(i)}^i)$$ &mdash; множества векторов (изображений) в каждом классе. Посчитаем такую матрицу:
...
где $$S_i$$ &mdash; матрица рассеивания для класса $$i$$, который определяется следующим образом:

$$
\begin{equation} \label{scatter-mat} \tag{1.6}
S_i = \sum_{\boldsymbol{x} \in D_i} {
...
**2. Подсчёт матрицы $$S_B$$ рассеивания между классами.** Считается таким образом:
